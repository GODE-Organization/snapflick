  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="relative">
        <div
          className={`w-full rounded-xl bg-surface-off-white px-4 py-3 text-body-md text-on-background ${mono ? "font-mono" : ""}`}
        >
          {value || <span className="text-on-surface-variant opacity-50">No detectado</span>}
        </div>
        {confidence && (
          ...
          <div className="flex flex-col gap-6 xl:col-span-7">
            <div className="flex flex-col gap-8 rounded-[24px] border border-outline-variant/30 bg-surface-container-lowest p-8 shadow-sm">
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                <Field label="Nombre" value={current.sheet.name} confidence={current.sheet.confidence} />
                <Field label="Marca" value={current.sheet.brand ?? ""} confidence={current.sheet.confidence} />
              </div>
              <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
                <Field label="Presentación" value={current.sheet.presentation ?? ""} />
                <div className="md:col-span-2">
                  <Field label="Categoría (auto)" value={current.sheet.category ?? "Sin categoría"} mono />
                </div>
              </div>
              <Field label="Descripción" value={current.sheet.description} confidence={current.sheet.confidence} />
              {current.sheet.keywords.length > 0 && (
                <div className="flex flex-col gap-2">
                  <label className="font-mono text-label-sm uppercase tracking-wider text-on-surface-variant">
                    Palabras clave
                  </label>
                  <div className="flex flex-wrap gap-2">
                    {current.sheet.keywords.map((k) => (
                      <span key={k} className="rounded-full bg-surface-container px-3 py-1 font-mono text-label-sm text-on-surface">
                        {k}
                      </span>
                    ))}
                  </div>
              ...
              {current.sheet.barcode && <Field label="Código de barras" value={current.sheet.barcode} mono />}
              {current.sheet.notes && (
                <div className="flex items-start gap-2 rounded-lg bg-surface-container p-4 text-on-surface-variant">
                  <Icon name="info" className="mt-0.5 shrink-0" />
                  <p className="text-body-md italic">{current.sheet.notes}</p>
                </div>
              )}
            </div>
